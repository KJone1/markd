...
describe("Markd shell", () => {
  ...
  it("reselecting active HTML resets the iframe to the selected file", async () => {
    const activeFile = {
      kind: "html" as const,
      path: "index.html",
      content: "<!doctype html>",
    };
    const openWorkspaceFile = vi.fn().mockResolvedValue(activeFile);
    vi.stubGlobal("bindings", {
      getAppInfo: vi.fn().mockResolvedValue({
        name: "Markd",
        platform: "darwin",
    ...
    await fireEvent.click(screen.getByRole("treeitem", { name: "index.html" }));

    await waitFor(() => {
      expect(screen.getByTitle("Previewing index.html")).not.toBe(firstFrame);
    });
    expect(openWorkspaceFile).toHaveBeenCalledWith("index.html");
    expect(
  ...
  it("opens the transient tree with Cmd+P and routes one active file", async () => {
    ...
    expect(await screen.findByRole("dialog", { name: "Open a file" }))
      .toBeTruthy();
    await fireEvent.keyDown(screen.getByRole("tree"), { key: "ArrowRight" });
  ...
  it("keeps the current file open when its save handoff rejects selection", async () => {
    ...
    vi.stubGlobal("bindings", {
      ...
      getWorkspaceNavigation: vi.fn().mockResolvedValue({
        rootPath: "/Users/example/Notes",
        entries: [
          { kind: "file", name: "current.ts", path: "current.ts" },
          { kind: "file", name: "other.ts", path: "other.ts" },
        ],
        activeFile: {
          kind: "code",
          path: "current.ts",
    ...
    }).markdBeforeWorkspaceSwitch = handoff;
    await fireEvent.keyDown(window, { key: "p", metaKey: true });
    await fireEvent.click(screen.getByRole("treeitem", { name: "other.ts" }));
